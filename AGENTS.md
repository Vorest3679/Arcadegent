...
- 前端依赖由 pnpm 管理，版本固定在 `apps/web/package.json`。前端命令在 `apps/web/` 执行：`pnpm run build`、`pnpm run test:e2e`。E2E 使用 `apps/web/playwright.config.ts`，启动本地 Vite 服务并保留失败 trace。
...
1. 在与工作类型对应的分支开发，例如 `feat/<简述>`、`debug/<简述>` 或 `fix/<简述>`；不要直接在 `main` 上开发；不要使用`codex/<...>`这样的新建分支，只允许在上述三个分支内提交。
